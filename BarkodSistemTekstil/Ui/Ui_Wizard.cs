        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            ...
            else if (level == 1 && deepUno == 2 && deepDos == 0)
            {
                ...
                else
                {
                    ...
                    Report rpt = new Report();
                    rpt.NewApp("24SaatRapor");
                    rpt.Visibility = true;
                    rpt.Border = true;
                    rpt.PageMarginLeft = 10;
                    rpt.PageMarginRight = 10;
                    rpt.CenterHeaderText(DateTime.Now.ToShortDateString());

                    progressBar1.Visible = true;
                    progressBar1.Maximum = query.Count();

                    rpt.ColumnsTitle(0, 0, true, "İşlem Tutarı");
                    rpt.ColumnsTitle(0, 1, true, "İşlem Tipi");
                    rpt.ColumnsTitle(0, 2, true, "Yapılan Toplam İndirim");
                    rpt.ColumnsTitle(0, 3, true, "Yapılan Ürün İndirimi");
                    ...
                    foreach (var item in query)
                    {
                        ...
                        rpt.WriteCell(row, 0, item.TotalPrice.ToString());
                        rpt.WriteCell(row, 1, item.SubTboPaymentMethod.PaymentMethodName);
                        rpt.WriteCell(row, 2, item.TotalDiscount.ToString());
                        rpt.WriteCell(row, 3, islemeyapilantotalürüniskontosu.ToString());
                        islemeyapilantotalürüniskontosu = 0;
                        row++;
                        progressBar1.PerformStep();
                    }
                    Toplam += (Veresiye + Nakit + Kredi);
                    rpt.MergeCell(row + 2, 0, row + 2, 2);
                    rpt.WriteRange(row + 2, 0, row + 2, 2, "Toplam :" + Toplam);
                    rpt.MergeCell(row + 4, 0, row + 4, 2);
                    rpt.WriteRange(row + 4, 0, row + 4, 2, "Toplam Islem Sayisi :" + (krediIslemSayisi+nakitIslemSayisi+veresiyeIslemSayisi));

                    rpt.WriteCell(row + 3, 0, "Nakit :" + Nakit+" TL");
                    rpt.WriteCell(row + 3, 1, "Kredi :" + Kredi+" TL");
